...
df_metadata = read_excel(
  ...
  mutate(var_id = recode(var_label,
...
assertthat::assert_that(
  all(df_metadata$var_id %in% colnames(sf_data)),
  msg = "Metadata variable names do not match data variable names"
)
assertthat::assert_that(
  all(sf_data %>% names() %>% discard(~.x%in%c("district","geometry"))  %in% df_metadata$var_id),
  msg = "Metadata variable names do not match data variable names"
)

save(sf_data, df_metadata, file = here("App/shiny/phl-council-health-app-v1/data/app_v0.1.RData"))
