...
geojson_districts %>% write_rds(here("App/shiny/phl-council-health-app-v1/data/geojson_districts.rds"))
```
...
df_metadata = read_excel(
  ...
  mutate(var_name = recode(var_label,
...
assertthat::assert_that(
  all(df_metadata$var_name %in% colnames(df_data)),
  msg = "Metadata variable names do not match data variable names"
)
assertthat::assert_that(
  all(df_data %>% names() %>% discard(~.x%in%c("district","geometry"))  %in% df_metadata$var_name),
  msg = "Metadata variable names do not match data variable names"
)

```

Let's also tidy the data for the dashboard

```{r}
df_data = df_data %>% 
  sf::st_drop_geometry() %>% 
  pivot_longer(-district, names_to = 'var_name') %>% 
  left_join(df_metadata %>% select(var_name, var_label))
